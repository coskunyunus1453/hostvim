    ...
    <div class="hv-container">
        <div class="docs-layout">
            <aside class="docs-layout__aside hidden lg:block">
                <x-docs.sidebar :nav-roots="$navRoots" />
            </aside>
                ...
                @else
                    <div class="space-y-5">
                        @foreach ($roots as $root)
                            <section class="rounded-2xl border border-slate-200/90 bg-white/90 p-6 dark:border-slate-800 dark:bg-slate-900/60">
                                ...
                                @if ($root->children->isNotEmpty())
                                    <ul class="mt-4 grid gap-2 border-t border-slate-200/90 pt-4 sm:grid-cols-2 dark:border-slate-800/80">
                                        @foreach ($root->children as $child)
                                            <li>
                                                <a href="{{ route('docs.show', $child->slug) }}" class="hv-link text-sm font-medium">
                                                    {{ $child->title }}
                                                </a>
                                            </li>
                                        @endforeach
