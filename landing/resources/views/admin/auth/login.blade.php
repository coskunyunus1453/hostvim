...
<body class="admin-shell min-h-full bg-slate-50 font-sans text-slate-800 antialiased dark:bg-slate-950 dark:text-slate-200">
    ...
    <div class="relative z-10 mx-auto flex min-h-screen max-w-md flex-col justify-center px-5">
        ...
        <div class="admin-card p-6 shadow-xl backdrop-blur">
            <h1 class="text-lg font-semibold tracking-tight text-slate-900 dark:text-slate-50">Giriş yap</h1>
            <p class="admin-muted mt-1 text-xs">Sadece yetkili yönetici hesapları erişebilir.</p>
            ...
            <form method="POST" action="{{ route('admin.login.store') }}" class="mt-6 space-y-4">
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus autocomplete="username" class="admin-field mt-1" />
                    @error('email')
                ...
                <div>
                    ...
                    <input id="password" name="password" type="password" required autocomplete="current-password" class="admin-field mt-1" />
                    @error('password')
                        <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <button type="submit" class="admin-btn-primary-lg w-full">Giriş yap</button>
            </form>
        </div>
