...
<div class="mx-auto max-w-3xl space-y-8">
    ...
    <form method="POST" action="{{ route('admin.site-settings.update') }}" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="admin-form-panel space-y-5 !shadow-none">
            ...
            <div>
                <p class="admin-label mb-2">Üst menü marka görünümü</p>
                <p class="mb-3 text-[11px] text-slate-500">Ön yüz header’da logo ve site adının nasıl gösterileceğini seçin. Altbilgi bu ayardan etkilenmez.</p>
                <div class="space-y-2">
                    @foreach (\App\Services\LandingAppearance::HEADER_BRAND_MODE_LABELS as $modeKey => $modeLabel)
                        <label class="admin-radio-tile">
                            <input type="radio" name="header_brand_mode" value="{{ $modeKey }}" class="mt-1" @checked(old('header_brand_mode', $headerBrandMode ?? 'both') === $modeKey)>
                            <div>
                                <div class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $modeLabel }}</div>
                                <div class="mt-0.5 text-xs text-slate-600 dark:text-slate-500">
                                    @if ($modeKey === 'both')
                                        Logo (veya baş harf) ile site adı ve slogan birlikte gösterilir.
                                    @elseif ($modeKey === 'logo_only')
                                        Yalnızca logo; logo yoksa baş harf kutusu kullanılır.
                                    @else
                                        Logo gizlenir; site adı ve slogan metin olarak kalır.
                                    @endif
                                </div>
                            </div>
                        </label>
