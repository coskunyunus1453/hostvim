...
class LayoutComposer
{
    ...
    protected function buildPayload($view): array
    {
        ...
        return [
            ...
            'footerMenu' => $this->cache->remember('layout:footer_menu', fn () => Menu::where('location', 'footer')->with(['activeRootItems' => fn ($q) => $q->with(['activeChildren.page'])])->first()),
            'footerBottomMenu' => $this->cache->remember('layout:footer_bottom_menu', fn () => Menu::where('location', 'footer_bottom')->with('activeItems.page')->first()),
