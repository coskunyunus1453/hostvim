...
<div class="flex items-center gap-1.5 sm:gap-2">
    @if($isAdmin)
        <a href="{{ url('/admin') }}" class="btn-secondary hidden px-3 py-2 text-xs sm:inline-flex sm:text-sm">
    ...
    @elseif($loggedIn)
        <a href="{{ $accountHref }}" class="btn-secondary hidden px-3 py-2 text-xs sm:inline-flex sm:text-sm {{ request()->routeIs('account.*') ? 'ring-2 ring-hv-primary/40' : '' }}">
        ...
        <a href="{{ $accountHref }}" class="btn-secondary inline-flex px-2.5 py-2 text-xs sm:hidden" aria-label="Hesabım">
