...
<section class="py-10">
    <div class="mx-auto max-w-4xl px-4 lg:px-8">
        <h1 class="text-2xl font-extrabold text-hv-text">Sipariş Özeti</h1>
        <p class="mt-1 text-hv-muted">Bilgileri kontrol edip sepete ekleyin.</p>
        ...
        <div class="rounded-2xl border border-hv-border bg-hv-elevated p-6 shadow-sm">
            <h2 class="font-bold text-hv-text">{{ $product->name }}</h2>
            <dl class="mt-4 space-y-3 text-sm">
                <div class="flex justify-between">
                    <dt class="text-hv-muted">Alan adı</dt>
                    <dd class="font-semibold">{{ $config['domain_name'] }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-hv-muted">Alan adı seçeneği</dt>
                ...
                <div class="flex justify-between">
                    <dt class="text-hv-muted">Ödeme sıklığı</dt>
                    <dd class="font-semibold">{{ \App\Support\BillingCycle::label($config['billing_cycle']) }}</dd>
                </div>
                <div class="flex justify-between border-t border-hv-border pt-3">
                    <dt class="text-hv-muted">Hosting</dt>
                    <dd class="font-semibold">₺{{ number_format($pricing['final'], 2, ',', '.') }}</dd>
                </div>
                @foreach($addonLines as $line)
                    <div class="flex justify-between">
                        <dt class="text-hv-muted">{{ $line['name'] }}</dt>
                ...
                @endif
                <div class="flex justify-between border-t border-hv-border pt-4 text-base">
                    <dt class="font-bold text-hv-text">Hosting toplamı</dt>
                    <dd class="font-bold text-hv-primary">₺{{ number_format($hostingTotal, 2, ',', '.') }}</dd>
                </div>
            </dl>
        </div>
        ...
        <form method="POST" action="{{ route('hosting.configure.complete') }}" class="mt-6 flex justify-between gap-3">
            @csrf
            <a href="{{ route('hosting.configure.options') }}" class="rounded-xl border border-hv-border px-6 py-3 font-semibold text-hv-text">Geri</a>
