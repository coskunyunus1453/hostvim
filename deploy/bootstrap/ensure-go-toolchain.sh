#!/usr/bin/env bash
# Panelze: engine/go.mod ile uyumlu Go toolchain (resmi go.dev paketi).
# Kaynak: https://go.dev/dl/ — müşteri sunucusunda apt'teki eski golang-go yerine kullanılır.
#
# Ortam (isteğe bağlı):
#   PANELZE_GO_VERSION=1.22.3   # engine/go.mod ile uyumlu (eski: PANELSAR_GO_VERSION)

ensure_go_toolchain() {
  local want="${PANELZE_GO_VERSION:-${PANELSAR_GO_VERSION:-1.23.4}}"
  ...
  if command -v curl >/dev/null 2>&1; then
    if [[ "${PANELZE_INSECURE_DOWNLOAD:-${PANELSAR_INSECURE_DOWNLOAD:-0}}" == "1" ]]; then
