...
update_env "DB_DATABASE" "panelze"
update_env "DB_USERNAME" "panelze"
...
update_env "MYSQL_PROVISION_USERNAME" "panelze_provision"
...
echo "==> Bağlantı testi..."
mysql -u panelze -p"${PANEL_DB_PASS}" -h 127.0.0.1 -e "USE panelze; SELECT 'panel_db ok' AS status;"
mysql -u panelze_provision -p"${MYSQL_PROVISION_PASS}" -h 127.0.0.1 -e "SELECT 'provision ok' AS status;"
