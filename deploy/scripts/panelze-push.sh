#!/usr/bin/env bash
#
# Panelze — Mac / geliştirici: değişiklikleri main dalına push eder.
#
#   bash panelze-push
#   bash panelze-push "Panel: nginx düzeltmesi"
#
# Ortam:
#   PANELZE_DEPLOY_BRANCH=main   # sunucunun çekeceği dal
#   PANELZE_SKIP_COMMIT=1        # yalnızca mevcut commit'leri push et
#
set -euo pipefail
...
MSG="${1:-}"
SKIP_COMMIT="${PANELZE_SKIP_COMMIT:-${PANELZE_SKIP_COMMIT:-0}}"
...
if [[ "$SKIP_COMMIT" != "1" ]]; then
  if [[ -n "$(git status --porcelain)" ]]; then
    if [[ -z "$MSG" ]]; then
      MSG="panelze deploy $(date '+%Y-%m-%d %H:%M')"
    fi
...
echo ""
echo "Tamam. Sunucuda (SSH — Mac'te panelze-deploy ÇALIŞTIRMAYIN):"
