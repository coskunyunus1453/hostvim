...
panelze_deploy_common_loaded() { :; }

panelze_resolve_home() {
...
panelze_www_data_home() {
  local panel_root="${1:-${PANEL_ROOT:-}}"
  if [[ -n "$panel_root" ]]; then
    printf '%s\n' "$panel_root"
    return 0
  fi
  printf '%s\n' "$(panelze_resolve_home)/panel"
}

panelze_ensure_www_data_config_dir() {
  local panel_root
  panel_root="$(panelze_www_data_home "$1")"
...
panelze_run_artisan() {
  ...
  panelze_ensure_www_data_config_dir "$panel_root"
